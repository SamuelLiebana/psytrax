...
def hyperOpt(dat, hyper, n_params, log_lik_fns, optList, E0=None,
    ...
    pbar = tqdm(desc='Fitting', unit='cycle', disable=not show_progress)
    ...
    while True:
        ...
        Hess, logEvd, llstruct = getMAP(
            ...
            E0=current_E0, method=method, showOpt=0, pbar=pbar,
        )
        pbar.update(1)
        pbar.set_postfix({'log_evd': f'{logEvd:.3f}', 'best': f'{best_logEvd:.3f}' if best_logEvd else '—'})
        ...
        if not current_jump:
            # Build optVals from best_hyper and break
        ...
        # Decouple prior/likelihood for fast hyper gradient
...
def _hyperOpt_lossfun(optVals, keywords):
    ...
    pT, lT = getPosteriorTerms(E_flat, dat=dat, hyper=hyper,
                               log_lik_fns=log_lik_fns, method=method)
