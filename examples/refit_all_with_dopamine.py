...
Optional flags:
    ...
    --hess-calc {None|Weights|All}
                                Hessian level (default: All — also gives
                                hyperparameter credible intervals)
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--hess-calc', type=str, default='All',
...
def main() -> int:
    ...
    for i, f in enumerate(files, 1):
        ...
        try:
            ...
            summary.append((mouse, None, None, str(exc)))
            continue
        ...
        summary.append((mouse, log_evd, elapsed, None))

    total = time.time() - t_total
    ok    = sum(1 for s in summary if s[3] is None)
    fail  = sum(1 for s in summary if s[3] is not None)
    print(f'\n[done] {ok} succeeded, {fail} failed in {total:.0f}s '
          f'(out: {OUT_DIR})')
    if fail:
        print('Failures:')
        for mouse, _, _, err in summary:
            if err is not None:
                print(f'  {mouse}: {err}')
